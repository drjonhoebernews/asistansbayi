...
import PoliceOzellikListPage from "@/app/(hydrogen)/policeoz/ozellikler/page";
...
export default function PoliceOzellikListTable() {
    ...
    useEffect(() => {
        async function fetchData() {
            setLoading(true);
            try {
                const response = await apiHelper.get<CustomData[]>('paketler/police-ozellikleri');
                if (response.data) {
    ...
    const handleDeletee = async (id: string | number) => {
        setLoading(true);
        try {
            await apiHelper.delete(`/paketler/police-ozelligi/${id}`);
            toast.success(
        ...
        tableFooter={
          ...
          >
            <Button size="sm" className="dark:bg-gray-300 dark:text-gray-800">
              ...
              {selectedRowKeys.length > 1 ? 'Özellik' : 'Özellik'}
            </Button>
          </TableFooter>
        }
      />

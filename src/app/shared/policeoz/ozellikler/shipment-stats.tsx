...
import MarkaListPage from "@/app/(hydrogen)/marka/araclar/page";
...
export default function PoliceOzellikStats({ className }: IndexProps) {
  return (
    <SimpleBar>
      <div className="grid grid-flow-col gap-5">
        {data.map((item) => (
          <PoliceOzellikStat key={item.id} {...item} />
        ))}
      </div>
    </SimpleBar>
  );
...
export function PoliceOzellikStat({ count, icon, label }: StatType) {
  return (
    <div
      className={cn(

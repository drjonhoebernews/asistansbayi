...
import MarkaListPage from "@/app/(hydrogen)/model/araclar/page";
...
export default function ModelStats({ className }: IndexProps) {
  return (
    <SimpleBar>
      <div className="grid grid-flow-col gap-5">
        {data.map((item) => (
          <ModelStat key={item.id} {...item} />
        ))}
      </div>
    </SimpleBar>
  );
...
export function ModelStat({ count, icon, label }: StatType) {
  return (
    <div
      className={cn(

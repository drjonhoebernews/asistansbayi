  ...
  return (
    ...
    >
      <div className="h-80 w-full max-w-full justify-around gap-6 @sm:pt-3 @7xl:gap-8 md:h-[26rem] 3xl:h-[22rem]">
        ...
        {/* {data.map((item) => (
          <div key={item.name} className="grid gap-6 text-center">
            <CircleProgressBar
              percentage={item.percentage}
              size={120}
              stroke="#D0B8E9"
              strokeWidth={8}
              progressColor="#4C2889"
              // gradientColor="#B80590"
              useParentResponsive={true}
              label={<Text tag="h6">{item.value}</Text>}
            />
            <Text className="font-semibold text-gray-900 @7xl:text-base dark:text-gray-700">
              {item.name}
            </Text>
          </div>
        ))} */}
        ...
        {data.map((item, index) => (
          <div key={item.name} className="grid gap-2">
            <div className="flex items-center">
              ...
              />
              <Text tag="span" className=" whitespace-nowrap">
                {item.name}
              </Text>
            </div>
            <p className="ms-5 font-medium">

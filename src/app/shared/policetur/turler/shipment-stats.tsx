...
export default function PoliceTurlerStats({ className }: IndexProps) {
  return (
    <SimpleBar>
      <div className="grid grid-flow-col gap-5">
        {data.map((item) => (
          <PoliceTurlerStat key={item.id} {...item} />
        ))}
      </div>
    </SimpleBar>
  );
...
export function PoliceTurlerStat({ count, icon, label }: StatType) {
  return (
    <div
      className={cn(

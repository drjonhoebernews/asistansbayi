...
export default function PoliceTurlerListTable() {
    ...
    useEffect(() => {
        async function fetchData() {
            setLoading(true);
            try {
                const response = await apiHelper.get<CustomData[]>('paketler/servis-turleri');
                if (response.data) {
    ...
    const handleDeletee = async (id: string | number) => {
        setLoading(true);
        try {
            await apiHelper.delete(`/paketler/servis-turu/${id}`);
            toast.success(
        ...
        tableFooter={
          ...
          >
            <Button size="sm" className="dark:bg-gray-300 dark:text-gray-800">
              ...
              {selectedRowKeys.length > 1 ? 'Türler' : 'Tür'}
            </Button>
          </TableFooter>
        }
      />

...
const FormFormSchema = z.object({
    name: z.string().min(1, { message: 'Tür gerekli' }),
});
...
export const EditodalModalView: React.FC<Props> = ({ id }) => {
    ...
    const fetchInitialData = async () => {
        setIsFetching(true);
        try {
            const response = await apiHelper.get(`/paketler/servis-turu/${id}`);
    ...
    const onSubmit: SubmitHandler<FormInfoFormTypes> = async (data) => {
        ...
        try {
            ...
            if (id) {
                response = await apiHelper.put(`/paketler/servis-turu/${id}`, formData, {
                    headers: {
  ...
  return (
    <div className="flex flex-col gap-4 text-gray-700">
        <Input
            type="text"
            label="Tür"
            placeholder="Örnek: Hususi Otomobil"

...
export const getColumns = () => [
  {
    title: <span className="ms-6 block">Origin</span>,
    dataIndex: 'origin',
    key: 'origin',
    width: 200,
    render: ({
  ...
  {
    title: <span className="block">Destination</span>,
  ...
  {
    title: <span className="block">Date</span>,
    dataIndex: 'date',
    key: 'date',
    width: 200,
  ...
  },
  {
    ...
    key: 'weight',
    width: 200,
    render: (weight: string) => <span className="block">{weight}</span>,
  },
];
...
export default function RecentShipments({data, loading}: PersonalProps) {
  if (loading) {
    return <>
      <Spinner size={"xl"}/>
    </>;
  }
  console.log('alt bayi datası datası',data)
  return (
    <BasicTableWidget
      title="Alt Bayileri"
      className={cn(

...
export const FileInput = ({
  ...
  function handleFileUpload() {
    if (files.length) {
      console.log('Yüklenen dosya:', files);
      toast.success(<Text tag="b">Dosya başarıyla eklendi</Text>);

      setTimeout(() => {
        closeModal();
      }, 200);
    } else {
      toast.error(<Text tag="b">Lütfen dosya ekleyin</Text>);
    }
  }
  ...
  return (
    <div className={className}>
      ...
      {files.length > 1 ? (
        <Text className="mb-2 text-gray-500">{files.length} dosya</Text>
      ) : null}
      ...
      <div className="mt-4 flex justify-end gap-3">
        ...
        <Button className="w-full" onClick={() => handleFileUpload()}>
          <PiArrowFatLineUpBold className="me-1.5 h-[17px] w-[17px]" />
          {btnLabel}
        </Button>
      </div>
    </div>

...
export default function IlListTable() {
    ...
    useEffect(() => {
        async function fetchData() {
            setLoading(true);
            try {
                const response = await apiHelper.get<CustomData[]>('sehir/iller');
                if (response.data) {
    ...
    const handleDeletee = async (id: string | number) => {
        setLoading(true);
        try {
            await apiHelper.delete(`/sehir/il/${id}`);
            toast.success(
        ...
        tableFooter={
          ...
          >
            <Button size="sm" className="dark:bg-gray-300 dark:text-gray-800">
              ...
              {selectedRowKeys.length > 1 ? 'IL' : 'IL'}
            </Button>
          </TableFooter>
        }
      />
